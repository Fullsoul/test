...
<script type="text/javascript">
    $(function () {
        ...
        $("#firstCategoryId").change(function () {
            ...
            //初始化二级目录
            $.get('${pageContext.request.contextPath}/categorys/'+id,function (obj) {
...
<script type="text/javascript">
    ...
    layui.use(['table','form','element'],function () {
        ...
        tableIns=table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'publishStatus', title: '上架/下架', width: 110, sort: true,templet:function (obj) {//obj表示当前的对象
                        ...
                        var r="";
                        if(f==1){
                            r = " <input type=\"checkbox\" value='" + productId + "' name=\"flag\" checked='checked' lay-skin=\"switch\" lay-filter=\"active1\" lay-text=\"已上架|已下架\">";
                        }else {
                            r = "<input type=\"checkbox\" name=\"flag\" value='" + productId + "'  lay-skin=\"switch\" lay-filter=\"active1\" lay-text=\"已上架|已下架\">";
            ...
            {field: 'auditStatus', title: '审核', width: 110, sort: true,templet:function (obj) {//obj表示当前的对象
                    ...
                    var r="";
                    if(f==1){
                        r = " <input type=\"checkbox\" value='" + productId + "' name=\"flag\" checked='checked' lay-skin=\"switch\" lay-filter=\"active2\" lay-text=\"已审核|未审核\">";
                    }else {
                        r = "<input type=\"checkbox\" name=\"flag\" value='" + productId + "'  lay-skin=\"switch\" lay-filter=\"active2\" lay-text=\"已审核|未审核\">";
        ...
        table.on('toolbar(test)', function(obj){
            ...
            switch(obj.event){
                case 'add':
                    ...
                    location.href='${pageContext.request.contextPath}/jsp/product/productadd'
                ...
                case 'update':
                    if(data.length === 0){
                        ...
                        layer.msg('编辑 [id]：'+ checkStatus.data[0].productId);
                        window.location.href="${pageContext.request.contextPath}/product/modify?productId="+checkStatus.data[0].productId;
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'uoloadimages'){
                ...
                window.location.href="${pageContext.request.contextPath}/product/majorimage";
            ...
            if(layEvent === 'showProductDes'){
                ...
                $.ajax({
                    url:'${pageContext.request.contextPath}/product/querybyid',
                    data:{"productId":data.productId},
                    async:false,
                    cache:false,
                    success:function (obj) {
                        $("#edit_productId").val(obj.productId);
                        $("#edit_productName").val(obj.productName);
                    }
                ...
                layer.open({
                    ...
                    yes: function (index, layero) {
                        //alert($("#fm1").serialize());
                        $.ajax({
                            url: "${pageContext.request.contextPath}/product/editrole",
                            //表单序列化
                            data: $("#fm1").serialize(),
                            success: function (obj) {
                                if (obj.code==0) {
        ...
        //给激活   按钮切换事件
        form.on('switch(active1)', function (obj) {
            ...
            layer.open({
                ...
                yes:function (index) {//第一个按钮触发的事件
                    ...
                    //发送ajax请求 改变状态
                    $.ajax({
                        url:'${pageContext.request.contextPath}/product/active1',
                        data:{"productId":id,"publishStatus":status},
                        dataType:'json',
                        type:'post',
                        cache:false,
                        success:function (obj) {
        ...
        //给激活   按钮切换事件
        form.on('switch(active2)', function (obj) {
            //obj.elem.checked 获得复选框的状态  true选中  false 不选中
            // alert(obj.elem.checked);
            //获取复选框的状态  true表示激活  false 表示禁用
            var status = obj.elem.checked;
            ...
            layer.open({
                ...
                yes:function (index) {//第一个按钮触发的事件
                    ...
                    //发送ajax请求 改变状态
                    $.ajax({
                        url:'${pageContext.request.contextPath}/product/active2',
                        data:{"productId":id,"auditStatus":status},
                        dataType:'json',
                        type:'post',
                        cache:false,
                        success:function (obj) {
    ...
    function  deleBatch(ids) {
        $.ajax({
            url:'${pageContext.request.contextPath}/product/deletebatch',
            dataType:'json',
            data:{"ids":ids},
            type:'post',
            cache:false,
    ...
    function  delById(id,obj) {
        $.ajax({
            url:'${pageContext.request.contextPath}/product/deleterole/'+id,
            dataType:'json',
            type:'DELETE',
            cache:false,
            success:function (res) {
...
<div style="display: none;margin-top: 10px;width: 480px" id="dvlay">
    <form class="layui-form"  id="fm1"  action="${pageContext.request.contextPath}/product/domodify" method="post">
        <div class="layui-form-item">
            <label class="layui-form-label">商品序号</label>
            <div class="layui-input-block">
                <input type="text" name="productId" id="edit_productId"  readonly="readonly" autocomplete="off"
        ...
        <div class="layui-form-item">
            <label class="layui-form-label">商品名称</label>
            <div class="layui-input-block">
                <input type="text"  lay-verify="required"  name="productName" id="edit_productName"  autocomplete="off" class="layui-input">
            </div>
        </div>
